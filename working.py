...
====================''')
            ...
            yes/no?''')
                    ...
                    ppap=input("enter your bookname:")
                    w1=y1.lower()
                    ...
                    w2=ppap.lower()
                    cursor.execute("SELECT * FROM books")
                    lst21=cursor.fetchall()
                    mjo=[]
                    
                    
                    for i in range(0,len(lst21)):
                        mjo.append(list(lst21[i]))
                    
                    for j in range(0,len(mjo)):
                        b=mjo[j][0]
                        a=mjo[j][0].lower()
                        if a==w2:
                            count=mjo[j][2]+1
                            cursor.execute("UPDATE books SET copies='{}' WHERE Name='{}'".format(count,b))
                            mc.commit()
                        else:
                            pass
